...
const MisMascotas = ({ usuario, isAdmin }) => {
    ...
    try {
      if (isAdmin) {
        ...
        if (miCliente) {
          ...
          setForm(f => ({ ...f, id_cliente: miCliente.id_cliente }));
        }
      }
  ...
  const validarForm = () => {
    ...
    if (!form.edad || isNaN(form.edad) || form.edad < 0 || form.edad > 30) return 'Edad invalida (0-30)';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setForm({ nombre: '', sexo: 'M', edad: '', id_raza: '', id_cliente: isAdmin ? '' : form.id_cliente });
